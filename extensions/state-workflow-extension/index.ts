...
export const REGISTER_FUNCTION_HANDLER_EVENT =
...
export type ClientWorkflowStateDefinition = Omit<
  WorkflowDefinition["states"][string],
  "action" | "transitions" | "imagePath"
> & {
...
const parseStartArgs = (
  args: string,
): { workflowId?: string; autoRun: boolean } => {
...
const formatRunLines = (
  workflow: WorkflowDefinition,
  run: WorkflowRunState,
): string[] => {
  const state = run.currentStateId
    ? workflow.states[run.currentStateId]
    : undefined;
...
const formatTransitionOption = (
  transition: WorkflowDefinition["states"][string]["transitions"][number],
): string => {
...
export default function stateWorkflowExtension(pi: ExtensionAPI): void {
  ...
  const persistActiveWorkflow = (): void => {
    pi.appendEntry(ACTIVE_ENTRY, {
      workflowId: activeWorkflowId,
    } satisfies ActiveEntryData);
  };
  ...
  const executor = {
    execute: async ({
      state,
      run,
    }: ActionExecutionRequest): Promise<RawExecutionResult> => {
  ...
  const refreshWidget = async (ctx: ExtensionContext): Promise<void> => {
    ...
    ctx.ui.setWidget(WIDGET_ID, formatRunLines(workflow, run), {
      placement: "belowEditor",
    });
  };

  const requireActiveWorkflowId = (
    ctx: ExtensionCommandContext,
    args: string,
  ): string | undefined => {
    ...
    if (!workflowId) {
      ctx.ui.notify(
        "No active workflow. Use /workflow-start <id> first.",
        "warning",
      );
      return undefined;
    }
  ...
  const resolveManualTransitionId = async (
    ...
    if (transitions.length === 0) {
      ctx.ui.notify(
        "No selectable manual transitions are available.",
        "warning",
      );
      return undefined;
    }
    ...
    if (!ctx.hasUI) {
      ctx.ui.notify(
        "Multiple transitions are available. Use /workflow-next <transitionId>.",
        "warning",
      );
      return undefined;
    }
  ...
  const startWorkflow = async (
    ...
    if (!autoRun) {
      ctx.ui.notify(
        `Started workflow ${workflowId} at state ${run.currentStateId}`,
        "info",
      );
      return;
    }
  ...
  pi.events.on(START_WORKFLOW_EVENT, (payload) => {
    void (async () => {
      const {
        ...
        autoRun = false,
        ctx,
      } = payload as StartWorkflowPayload;
      try {
  ...
  pi.registerTool({
    ...
    promptSnippet:
      "Use workflow_next to confirm a manual workflow transition by its transitionId.",
    promptGuidelines: [
    ...
    parameters: Type.Object({
      transitionId: Type.String({
        description:
          "Manual workflow transition ID to select, for example retry-linter.",
      }),
    }),
    async execute(toolCallId, params) {
      ...
      const details = (
        value: WorkflowNextToolDetails,
      ): WorkflowNextToolDetails => value;
      ...
      if (!workflowId) {
        return {
          content: [
            {
              type: "text",
              text: "No active workflow. workflow_next was ignored.",
            },
          ],
      ...
      if (!transitionId) {
        return {
          content: [
            {
              type: "text",
              text: "workflow_next requires a non-empty transitionId.",
            },
          ],
  ...
  pi.registerTool({
    ...
    description:
      "Show state transition history for the active workflow or a specified workflow.",
    promptSnippet:
      "Use workflow_history to inspect the workflow state transition log before deciding the next step.",
    parameters: Type.Object({
      workflowId: Type.Optional(
        Type.String({
          description:
            "Workflow ID. When omitted, the active workflow is used.",
        }),
      ),
    }),
    async execute(_toolCallId, params) {
      ...
      if (!workflowId) {
        return {
          content: [
            {
              type: "text",
              text: "No active workflow. workflow_history was ignored.",
            },
          ],
      ...
      if (!workflow || !run) {
        return {
          content: [
            { type: "text", text: `Workflow ${workflowId} has no active run.` },
          ],
      ...
      return {
        content: [
          { type: "text", text: formatHistoryLines(workflow, run).join("\n") },
        ],
        details: {
          workflowId,
          status: run.status,
  ...
  pi.on("tool_execution_end", async (event, ctx) => {
    ...
    try {
      await chooseAgentAndRunEnteredState(
        pending.workflowId,
  ...
  pi.registerCommand("workflow-list", {
    ...
    handler: async (_args, ctx) => {
      ...
      const text = workflows
        .map(
          (workflow) =>
            `${workflow.id}${workflow.id === activeWorkflowId ? " *active" : ""}`,
        )
        .join("\n");
  ...
  pi.registerCommand("workflow-next", {
    ...
    handler: async (args, ctx) => {
      ...
      const transitionId = await resolveManualTransitionId(
        workflowId,
        parseWorkflowIdArg(args),
        ctx,
      );
      if (!transitionId) {
        return;
